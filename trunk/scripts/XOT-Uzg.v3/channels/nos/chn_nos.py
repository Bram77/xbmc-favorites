...
class Channel(chn_class.Channel):
    ...
    def ProcessExtendedPages(self, data):
        try:
            ...
            results = common.DoRegexFindAll('<a href="http://player.omroep.nl/(\?aflID=\d+&amp;md5=[a-f0-9]+)"[^>]*><[^>]+alt="bekijk uitzending: ([^(]+) \(([0-9-]*)\)" /></a>', data)
            for result in results:
    ...
    def UpdateVideoItem(self, item):
        ...
        # first we need to get the securitycode to obtain the info XML
        url = "http://player.omroep.nl/%s" % item.url
...
#===============================================================================
        ...
        #description
        description = common.DoRegexFindAll('<gids_tekst>(.*)</gids_tekst>', data)
        if description == []:
        ...
        if image == []: #no image
            ...
            item.thumb = self.CacheThumb(common.ConvertHTMLEntities(str(image[0][0]) + str(image[0][1])))
                        
        # get the mediaurl
        item.mediaurl = common.DoRegexFindAll('<stream[^>]+compressie_kwaliteit=.bb.[^>]+compressie_formaat=.wmv.[^>]*>([^<]*)</stream>', data)[0]
