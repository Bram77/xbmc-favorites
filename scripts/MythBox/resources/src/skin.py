...
class XBMC_SKIN(xbmcgui.Window):
    ...
    def getvalue(self,strval):
        ...
        try:
            ...
            log.error( "Possible skin error: problem evaluating '%s'"%r )
            return r
...
# =============================================================================
class XBMC_SKIN_DIALOG(xbmcgui.WindowDialog):
    ...
    def loadskin(self,skinName):
        ...
        # load the skin XML
        skinXml = util.loadSkin(
            skinName, self.getWidth(), self.getHeight() )
        
        # TODO: Remove OS speicifc check
        if os.name != "nt":
            log.debug( "skinXml=[%s]"%skinXml )
